...
class FeatureWiz(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        preds = [x for x in list(X_sel) if x not in targets]
        ###   This is where we sort the columns to make sure that the order of columns doesn't matter in selection ###########
        max_cats = 10
        numvars = X_sel[preds].select_dtypes(include = 'number').columns.tolist()
        if self.verbose:
            print('    selecting %d numeric features for further processing...' %len(preds))
        catvars = left_subtract(preds, numvars)
        if len(catvars) > max_cats:
            if feature_gen:
        ...
        if self.feature_gen:
            if np.where('groupby' in self.feature_gen,True, False).tolist():
                if len(catvars) >= 1 and len(numvars) >= 1:
                    grp = Groupby_Aggregator(categoricals=catvars, aggregates=['mean'], numerics='all')
        ...
        if self.feature_gen:
            if np.where('interactions' in self.feature_gen,True, False).tolist():
                if len(catvars) > 1:
                    ...
                    print('Adding %s interactions between categorical_vars %s...' %(
                                        num_combos, catvars))
                    X_sel = FE_create_interaction_vars_train(X_sel, catvars)
                    combovars = left_subtract(X_sel.columns.tolist(), numvars)
        ...
        self.model_type, self.multi_label_type = analyze_problem_type(df[targets], targets)
        numvars = X_sel.columns.tolist()
        if not self.skip_sulov:
            numvars = FE_remove_variables_using_SULOV_method(df, numvars, self.model_type, targets,
                                 self.corr_limit, self.verbose, self.dask_xgboost_flag)
        if not self.skip_xgboost:
            print('Performing recursive XGBoost feature selection from %d features...' %len(numvars))
            features = FE_perform_recursive_xgboost(df[numvars], df[targets], self.model_type, 
                            self.multi_label_type, self.dask_xgboost_flag, self.verbose)
        else:
            features = copy.deepcopy(numvars)
